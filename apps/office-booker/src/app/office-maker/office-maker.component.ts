  ...
  getFacilitiesForDesk(deskId: number) {
    if (deskId != 0) {
      this.bookingService.getFacilitiesByDeskId(deskId).subscribe(res => {
        ...
        if (JSON.stringify(res) != "[]") {//split the parsed string to extract the varaiables we need
          ...
          const projectorsString = myArray[4].split(":")[1].replace(/\D/g, '');//change the extracted strings into numbers
